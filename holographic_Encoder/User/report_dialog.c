    ...
    TEXT_SetTextAlign(WM_GetDialogItem(hWin,GUI_ID_TEXT10),GUI_TA_VCENTER|GUI_TA_LEFT);
		...
		switch(reportType){
			...
			if(isOrCheckLogo[logoCount]==0) {
					...
					randData = rand()%(reportCount/workMode);
				...
				for(i=0;i<reportCount;i++) {
					...
					minValue=d1-(d2-d1)*0.05*workMode;
					maxValue=d2+(d2-d1)*0.05*workMode;
					fnRTC_GetTime(&getTime); 
					srand(getTime.Second+i);
...
//							if(rand()%2==0){ //偏大
//								distance = d2*0.1;
//								resultValue = d2+rand()%((int)(distance*1000))/1000.0;
//							}else { //偏小
//								distance = d1*0.1;
//								resultValue = d1-rand()%((int)(distance*1000))/1000.0;
//							}
//							randData = rand()%(reportCount/workMode)+base*(reportCount/workMode);
//						}
